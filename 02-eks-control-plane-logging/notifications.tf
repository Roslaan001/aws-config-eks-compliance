...
resource "aws_sns_topic" "compliance_alerts" {
  name = "eks-control-plane-logging-compliance-alerts"
}
...
resource "aws_cloudwatch_event_rule" "compliance" {
  name        = "eks-control-plane-logging-compliance-rule"
...
# 6. IAM Role assumed by AWS Chatbot (unique name for the logging stack)
resource "aws_iam_role" "chatbot" {
  name = "aws-config-logging-chatbot-role"
...
resource "aws_chatbot_slack_channel_configuration" "slack" {
  configuration_name = "eks-control-plane-logging-alerts"
