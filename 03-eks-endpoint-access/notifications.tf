...
resource "aws_sns_topic" "compliance_alerts" {
  name = "eks-endpoint-access-compliance-alerts"
}
...
resource "aws_cloudwatch_event_rule" "compliance" {
  name        = "eks-endpoint-access-compliance-rule"
...
# 6. IAM Role assumed by AWS Chatbot (unique name for the endpoint access stack)
resource "aws_iam_role" "chatbot" {
  name = "aws-config-endpoint-chatbot-role"
...
resource "aws_chatbot_slack_channel_configuration" "slack" {
  configuration_name = "eks-endpoint-access-alerts"
