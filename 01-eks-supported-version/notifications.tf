...
resource "aws_sns_topic" "compliance_alerts" {
  name = "eks-supported-version-compliance-alerts"
}
...
resource "aws_cloudwatch_event_rule" "compliance" {
  name        = "eks-supported-version-compliance-rule"
...
# 6. IAM Role assumed by AWS Chatbot
resource "aws_iam_role" "chatbot" {
  name = "aws-config-chatbot-role"
...
resource "aws_chatbot_slack_channel_configuration" "slack" {
  configuration_name = "eks-supported-version-alerts"
