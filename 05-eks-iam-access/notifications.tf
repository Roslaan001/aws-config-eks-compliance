...
resource "aws_sns_topic" "compliance_alerts" {
  name = "eks-iam-access-compliance-alerts"
}
...
# 6. IAM Role assumed by AWS Chatbot (unique name for the EKS IAM access stack)
resource "aws_iam_role" "chatbot" {
  name = "aws-config-iam-chatbot-role"
...
resource "aws_chatbot_slack_channel_configuration" "slack" {
  configuration_name = "eks-iam-access-alerts"
